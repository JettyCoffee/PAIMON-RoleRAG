...
这是实现目标的第一步，即如何构建一个既包含“事实”又包含“风格”的图谱。

**数据源：** 爬取原神的现有文本整理为基础数据集（由于是角色的扮演，数据的选择上选择有角色出场的剧情文本，这能避免引入其他无关文本导致的风格不同的问题）
...
- 在这之后还需要做一个语义歧义的检索，也就是需要分别识别角色节点间、非角色节点间是否是同一个object，以下是RoleRAG的方法参考：

- 第二个是角色和任务故事都在图内，这个时候的社区检测会是什么效果呢？**从多个角度生成多份社群摘要，把不同属性角度的内容从图里分离。**
